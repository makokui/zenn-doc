...
topics: [teams, azure, openai, copilot studio]
published: false
---

# はじめに
Microsoft Teams と Azure OpenAI の連携のはいくつか方法がります。本記事では、Copilot Studio を使用する方法を確認しています。本記事は 2024 年 3 月 18 日時点の内容を元に執筆しています。常に最新の情報を確認するようにしてください。

# Azure OpenAI の設定
### デプロイ
Copilot Studio から Azure OpenAI への接続には 2種類の方法があります。両方の方法を確認するため、2種類の Azure OpenAI をデプロイしておきます。Azure OpenAI のデプロイは本記事の本筋ではないないので割愛します。必要に応じて [公式ドキュメント](https://learn.microsoft.com/ja-JP/azure/ai-services/openai/how-to/create-resource?pivots=web-portal) を確認してください。  
![](/images/a52bd7b497cdf4/2024-03-18-06-34-37.png) 
ここでは、以下の 2つの Azure OpenAI をデプロイしました。  
...
- openai-makokui-prod-japaneast-002  
  gpt-35-turbo-16k をデプロイします。独自データは使用しません。
  ![](/images/a52bd7b497cdf4/2024-03-18-06-36-31.png)

### Azure OpenAI の応答確認
Azure AI Studio のチャット プレイグラウンドから応答を確認することもできますが、本記事では Copilot Studio から Azure OpenAI を呼び出すため、REST API を使用します。  
「チャット」 - 「コードの表示」からサンプルコードを取得します。言語に Curl を指定し、パラメータを確認します。　　
...
### Postman で確認
上記で確認したパラメータを使用して [Postman](https://www.postman.com/) で応答を確認してみます。  
...
Body に先ほど確認した request body を貼り付けます。Curl サンプルではダブルクォート (") がエスケープされているので、以下のように修正します。  
![](/images/a52bd7b497cdf4/2024-03-18-06-45-02.png)  
...
「生成型の回答を作成する」の中身を見てみると、先ほどの Azure OpenAI に接続されていることが確認できます。
![](/images/a52bd7b497cdf4/2024-03-18-08-07-58.png)



## 方法2: 独自データモデル未使用の Azure OpenAI を呼び出す
こちらが本記事のメインになります。今度は、Copilot Studio から Azure OpenAI を呼び出すフローを作成します。
...
事前検証時に Postman の Body に指定した JSON を貼り付けます。  
...
![](/images/a52bd7b497cdf4/2024-03-18-08-29-50.png)  

次に Azure OpenAI からの応答に関わる設定を行います。応答のデータ タイプに「サンプル データから」を指定し、「サンプル JSON からスキーマを取得する」を選択します。  
...
### Azure OpenAI からの応答データの加工
先の「HTTP要求の送信」処理で「Output」変数に応答データ (Json) が格納されています。そのままではチャットで返信させることができないので、Azure OpenAI からの応答メッセージを抜き出します。    
...
これで Microsoft Teams チャットより Azure OpenAI と会話することができるようになりました。
![](/images/a52bd7b497cdf4/2024-03-18-10-06-49.png)  


# まとめ
Microsoft Teams から Azure OpenAI に接続する、ということをテーマに確認を行いましたが、およそ 1 時間程度で構成することができました。セキュリティ強化やログ出力、例外対応など、まだ検討の余地は残っていますが、非常に簡単に Azure OpenAI と連携させることができます。
